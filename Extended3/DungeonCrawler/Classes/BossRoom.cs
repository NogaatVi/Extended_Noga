...
namespace BossRoomClass
{
    public class BossRoom : Room
    {
        Random random = new Random();

        public BossRoom(string name) : base(name)
        {
            Name = name;
        }

        public override void InitializeMonster() //initialize boss
        {
            BracketPutter("Boss Encounter");
            if (thisRoomMonster == null) 
            {
              thisRoomMonster = new EliteMonster(0, 0, 0, Monster.monsterCounter);
              Console.WriteLine("Elite Monster:Boss");
