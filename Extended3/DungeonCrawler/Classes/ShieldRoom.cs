...
namespace ShieldroomClass
{
    public class ShieldRoom : Room
    {
        ...
        public ShieldRoom(string name) : base(name)
        {
            Name = name;
        }
        ...
        public override void Encounter(Player player)// gain shiekd encounter
        {
            string action = "";
            BracketPutter(Name);
            ...
            Console.WriteLine($"{player.name}, you feel yoursel growing strong.\nYou have gained a shield!");
            gainShield(player);
        ...
        public void gainShield(Player player) 
        {
            player.shield += 1;
        }
    }
